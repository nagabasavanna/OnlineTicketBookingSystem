...
if($result = $mysqli->query($sqld)){
    ...
    if($result->num_rows > 0){
        while($row = $result->fetch_array()){
            if($i != 0)
                $destinationrid[$i++]=',';
            $destinationrid[$i++]=$row['rid'];
        }
        $result->free();
    } else{
        echo "hi";
        echo "Sorry! no Bus is from $o to $d $mysqli->error";
        die();
    }
} else{
    echo "outer else";
...
FROM busschedule bs
JOIN bshasroute a
ON bs.path like '%$o%$d%' AND a.rid IN (".implode($originrid).") AND bs.bsid=a.bsid";
...
FROM busschedule bs
JOIN bshasroute a
ON bs.path like '%$o%$d%' AND a.rid IN (".implode($destinationrid).") AND bs.bsid=a.bsid";
...
//so minmax is initialised by its null
// var_dump($minmax);
...
ON bs.path like '%$o%$d%' AND a.rid IN (".implode($originrid).") AND bs.bsid=a.bsid
JOIN bshasroute b
ON b.rid IN (".implode($destinationrid).") AND bs.bsid=b.bsid
JOIN  bshasroute c
...
//echo '<br>'.implode($originrid).'<br>'.implode($destinationrid).'<br>'.implode($minmax);
?>
